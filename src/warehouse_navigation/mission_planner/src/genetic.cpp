...
#include "warehouse_msgs/srv/localisation.hpp"
...
const cv::String mapFilename = "src/warehouse_navigation/mission_planner/config/maps/distance_test_field.png";
...
void initPopulation(int length, individual population[], std::vector<float> const &boundsData){
...
void mutate(individual &indi, std::vector<float> const &boundsData){
...
void runGeneticAlgorithm(int numPoints, std::vector<int16_t> const &inputData, std::vector<float> const &boundsData, float &xOut, float &yOut, float &thetaOut){
...
void localise(const std::shared_ptr<warehouse_msgs::srv::Localisation::Request> request, std::shared_ptr<warehouse_msgs::srv::Localisation::Response> response)
{   
    if(LOGGING){
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Incoming request with bounds %f<x<%f %f<y<%f %f<theta<%f", request->bounds.data[0], request->bounds.data[1], request->bounds.data[2], request->bounds.data[3], request->bounds.data[4], request->bounds.data[5]);
    }
    float x, y, theta;
    
    runGeneticAlgorithm(request->points.data.size()/2, request->points.data, request->bounds.data, x, y, theta);

    auto spatialMessage = std_msgs::msg::Float32MultiArray();
    spatialMessage.data.resize(3);
    spatialMessage.data[0] = x;
    spatialMessage.data[1] = y;
    spatialMessage.data[2] = theta;

    response->transform = spatialMessage;
    if(LOGGING){
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Sending back response: X = [%f] Y = [%f] Theta = [%f]", (float)(response->transform.data[0]), (float)(response->transform.data[1]), (float)(response->transform.data[2]));
    }
}
...
int main(int argc, char *argv[]){
    ...
    rclcpp::Service<warehouse_msgs::srv::Localisation>::SharedPtr service =
    node->create_service<warehouse_msgs::srv::Localisation>("localise", &localise);
